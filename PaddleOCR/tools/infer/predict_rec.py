...
class TextRecognizer(object):
    ...
    def resize_norm_img(self, img, max_wh_ratio):
        ...
        if 'SVTR' not in self.rec_algorithm:
            imgW = int((imgH * max_wh_ratio))
        if self.use_onnx:
            w = self.input_tensor.shape[3:][0]
            if w is not None and not isinstance(w, str) and w > 0:
                imgW = w
    ...
    def resize_norm_img_svtr(self, img,
                        image_shape,
                        padding=True,
    ...
    def resize_norm_img_svtr_wrong(self, img, image_shape):
    ...
    def __call__(self, img_list):
        print(self.rec_algorithm)
        print(self.rec_image_shape)
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            if self.rec_algorithm == "SRN":
                ...
                if self.use_onnx:
                    ...
                    input_dict = {}
                    input_dict[self.input_tensor.any_name] = norm_img_batch
                    self.predictor.infer(inputs=input_dict)
                    outputs = self.predictor.get_output_tensor(self.output_tensors.index).data
                    preds = outputs
                else:
...
def main(args):
    image_file_list, __ = get_image_file_list(args.image_dir)
    ...
    for ino in range(len(img_list)):
        if len(__) and rec_res[ino][0] == __[ino]:
            count += 1
            continue
        
        
        if len(__):
            logger.info("Predicts of {}:{}\n{}'{}'".format(valid_image_file_list[ino].split('/')[-1][:-4],
                rec_res[ino], ' '*49,__[ino]) ) 
        else:
            logger.info("Predicts of {}:{}\n".format(valid_image_file_list[ino].split('/')[-1][:-4],
                rec_res[ino]))
        #print(valid_image_file_list[ino])
    if args.benchmark:
