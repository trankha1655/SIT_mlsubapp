...
class TextDetector(object):
    def __init__(self, args, MBA_preprocess= None):
        self.args = args
        self.MBA_preprocess = MBA_preprocess
        ...
        self.predictor, self.input_tensor, self.output_tensors, self.config = utility.create_predictor(
            args, 'det', logger)
        #print('Shape', self.input_tensor.shape())

        if self.use_onnx:
            img_h, img_w = self.input_tensor.shape[2:]
            #print(self.input_tensor.shape[2:])
            if img_h is not None and img_w is not None and not isinstance(img_h, str) and not isinstance(img_w, str) and img_h > 0 and img_w > 0:
                pre_process_list[0] = {
                    'DetResizeForTest': {
    ...
    def __call__(self, img):
        #img = self.MBA_preprocess(img)
        # num = int(random.random()*100000)
        # cv2.imwrite('out/' + str(num)+'.jpg', img)
        ori_im = img.copy()
        data = {'image': img}
        ...
        if self.use_onnx:
            ...
            outputs = self.predictor.run(self.output_tensors, input_dict)
                ...
                input_dict = {}
                input_dict[self.input_tensor.any_name] = img
