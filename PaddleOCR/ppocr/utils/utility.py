...
def get_image_file_list(img_file, tail='.'):
    imgs_lists = []
    gt_list =[]
    ...
    if 'txt' in img_file:
        with open(img_file) as f:
            ...
            for line in f:
                gt = ''
                if not 'rec' and not 'crop' in img_file:
                    temp = line.split('/')[-1]
                    id = temp.split('_')[0]
                    #print(id)
                    list_ = glob.glob('../../MBA/images/MBA_image_test/Det_image/%s*.jpg' % id)
                else:
                    ...
                    if tail not in temp:
                        continue
                    gt = line.replace('\n','').split('\t')[1]
                    list_ = ['../../MBA/images/' + temp]
                    #print(temp, list_)
                #print(list_)
                for x in list_:
                    if x not in imgs_lists:
                        #pass
                        imgs_lists.append(x) 
                        gt_list.append(gt)
                # if os.path.isfile(temp):
                #     imgs_lists.append(temp)
                # else:
                #     print('Could not found %s' % temp)
        if len(imgs_lists) == 0:
            raise Exception("not found any img file in {}".format(img_file))
    ...
    if not len(gt_list):
        ...
        return imgs_lists, []
    imgs_lists, gt_list = zip(*sorted(zip(imgs_lists, gt_list)))
    #print(imgs_lists, gt_list)
    return imgs_lists, gt_list
